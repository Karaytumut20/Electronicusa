            ...
            return (
              <li key={cat.id} className="group relative">
                <Link
                  ...
                  className="flex items-center justify-between px-4 py-3 text-sm text-gray-600 hover:text-indigo-600 hover:bg-indigo-50/50 transition-all duration-200"
                >
                  ...
                  <ChevronRight
                    size={14}
                    className="text-gray-300 group-hover:text-indigo-400 opacity-50 group-hover:opacity-100 transition-all"
                  />
                </Link>
                ...
                {cat.subs && cat.subs.length > 0 && (
                  <div className="hidden group-hover:block absolute left-[100%] top-0 w-64 bg-white border border-gray-100 shadow-xl rounded-r-xl p-4 z-50 ml-2 animate-in fade-in slide-in-from-left-2 duration-200">
                    <h4 className="font-bold text-gray-800 mb-3 pb-2 border-b border-gray-100">
                      {cat.title}
                    </h4>
                    <div className="flex flex-col gap-1">
                      {cat.subs.map((sub: any) => (
                        <Link
                          ...
                          className="text-sm text-gray-500 hover:text-indigo-600 hover:bg-gray-50 px-2 py-1.5 rounded-md transition-colors"
                        >
                          {sub.title}
                        </Link>
                      ))}
