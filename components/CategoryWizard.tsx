...
import { ArrowLeft, Monitor, Smartphone, Camera, Headphones, Gamepad2, Plug, CheckCircle2, Laptop, Zap } from 'lucide-react';
...
export default function CategoryWizard({ categories }: { categories: any[] }) {
  ...
  const handleSelect = (item: any) => {
    // 1. Dinamik Marka Kontrolü (Bilgisayar veya Telefon kategorisi ise)
    // Kategori adı veya slug'ında "computer", "laptop", "phone" gibi ifadeler geçiyorsa marka seçimi yaptır.
    const isTech = item.slug.includes('computer') || item.slug.includes('laptop') || item.slug.includes('phone') || item.slug.includes('mobile');

    // Eğer alt kategorisi yoksa ve teknoloji kategorisiyse marka seçimi aç
    if ((!item.subs || item.subs.length === 0) && isTech && !isBrandSelection) {
        setIsBrandSelection(true);
        setHistory([...history, currentList]);
        setSelectedPath([...selectedPath, item.title]);

        let brands: string[] = computerBrands; // Default
        if (item.slug.includes('phone') || item.slug.includes('mobile')) brands = phoneBrands;
    ...
    if (isBrandSelection) {
        ...
        const catSlug = selectedPath[0].toLowerCase().includes('apple') ? 'apple' : 'computer'; // Basitleştirilmiş logic

        router.push(`/post-ad/details?cat=${catSlug}&path=${encodeURIComponent(finalPath)}&brand=${encodeURIComponent(brand)}`);
        return;
    }
          ...
          return (
            ...
            >
              <div className="w-14 h-14 rounded-lg flex items-center justify-center mr-4 bg-gray-100 text-gray-500 group-hover:bg-indigo-100 group-hover:text-indigo-700">
                {history.length === 0 && Icon ? Icon : (isLeaf ? <CheckCircle2 size={24} className="text-green-600" /> : <div className="text-sm font-bold opacity-50">{item.title ? item.title.substring(0,2).toUpperCase() : '?'}</div>)}
              </div>
