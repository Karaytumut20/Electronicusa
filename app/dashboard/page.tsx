...
export default function DashboardPage() {
  ...
  const statusData = [
    ...
    { name: 'Inactive', value: stats.length - activeCount - pendingCount, color: '#EF4444' }
  ];
  ...
  return (
    <div className="space-y-8">
        <div>
          ...
          <p className="text-slate-500 mt-1">Here is an overview of your account and listings.</p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <StatCard title="Total Ads" value={stats.length} icon={List} colorClass="text-blue-600" bgClass="bg-blue-50" />
            <StatCard title="Active" value={activeCount} icon={CheckCircle} colorClass="text-green-600" bgClass="bg-green-50" />
            <StatCard title="Pending" value={pendingCount} icon={Clock} colorClass="text-yellow-600" bgClass="bg-yellow-50" />
            <StatCard title="Total Views" value={totalViews} icon={Eye} colorClass="text-purple-600" bgClass="bg-purple-50" />
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="bg-white p-6 rounded-xl border border-gray-100 shadow-sm">
                <h3 className="font-bold text-slate-800 mb-6">Most Viewed Ads</h3>
                <div className="h-[300px]">
                  <ResponsiveContainer width="100%" height="100%">
