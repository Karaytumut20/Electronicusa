...
function MessagesContent() {
  ...
  if (!user) return <div className="p-10 text-center">Please login to view messages.</div>;
                        ...
                        >
                            ...
                            <div className="flex items-center gap-3 pl-4">
                                <div className="w-10 h-10 bg-gray-200 rounded-lg shrink-0 overflow-hidden border border-gray-200">
                                    {conv.ads?.image ? (
                                        <img src={conv.ads.image} className="w-full h-full object-cover" alt="Ad" />
                                    ) : (
                                        <div className="w-full h-full flex items-center justify-center text-gray-400"><ImageIcon size={16}/></div>
                                    )}
                                </div>
                                <div className="flex-1 min-w-0">
                                    <p className="text-[11px] font-bold text-gray-600 truncate">{conv.ads?.title || 'Deleted Ad'}</p>
                                    <p className="text-xs text-gray-500 truncate mt-0.5 opacity-80">Open message...</p>
                                </div>
                            </div>
                        </div>
                    );
        ...
        {activeConv ? (
            <>
                    ...
                    {messages.map((msg, index) => {
                        ...
                        return (
                            <div key={msg.id} className={`flex w-full ${isMe ? 'justify-end' : 'justify-start'}`}>
                                <div className={`relative max-w-[85%] sm:max-w-[70%] px-3 py-2 rounded-2xl text-sm shadow-sm ${isMe ? 'bg-[#dcf8c6] text-slate-900 rounded-tr-none' : 'bg-white text-slate-800 rounded-tl-none'} ${msg.is_pending ? 'opacity-70' : ''}`}>
                ...
                <form onSubmit={handleSend} className="bg-white p-3 border-t border-gray-200 z-20 flex items-center gap-2">
                    ...
                    />
                    <button type="submit" disabled={!inputText.trim() || sending} className="bg-indigo-600 text-white p-3 rounded-full hover:bg-indigo-700 transition-transform active:scale-95 disabled:opacity-50 flex items-center justify-center">
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-gray-400 p-8 text-center select-none">
                <MessageSquare size={64} className="opacity-40 mb-6"/>
                <h3 className="font-bold text-xl text-gray-600 mb-2">Select a Conversation</h3>
            </div>
        )}
      </div>
    </div>
