...
import { Trash2, Loader2, StarOff } from 'lucide-react';
  ...
  if (!user) {
      return (
        <div className="p-10 text-center bg-white border rounded-sm">
            <p className="text-gray-600 mb-4">Please login to see your favorites.</p>
            <Link href="/login" className="bg-blue-600 text-white px-6 py-2 rounded font-bold">Login</Link>
        </div>
      );
  }
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-sm shadow-sm p-6">
      <h1 className="text-xl font-bold text-[#333] mb-6 flex items-center gap-2">
        My Favorites <span className="text-sm font-normal text-gray-500">({ads.length})</span>
      </h1>
      ...
      {ads.length === 0 ? (
        <EmptyState
            ...
            description="Add ads to your favorites to track price changes."
